...
func sweepGroupRules(client *testClient) error {
	...
	rules, _, err := client.oktaClient.Group.ListGroupRules(context.TODO(), &query.Params{Limit: 300})
	if err != nil {
		return err
	}
	...
	for _, s := range rules {
		if s.Status == "ACTIVE" {
			if _, err := client.oktaClient.Group.DeactivateGroupRule(context.TODO(), s.Id); err != nil {
				errorList = append(errorList, err)
				continue
			}
		}
		if _, err := client.oktaClient.Group.DeleteGroupRule(context.TODO(), s.Id); err != nil {
			errorList = append(errorList, err)
		}
...
func doesGroupRuleExist(id string) (bool, error) {
	...
	_, response, err := client.Group.GetGroupRule(getOktaContextFromMetadata(testAccProvider.Meta()), id, nil)
