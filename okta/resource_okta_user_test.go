...
func sweepUsers(client *testClient) error {
	var errorList []error
	users, _, err := client.oktaClient.User.ListUsers(context.TODO(), &query.Params{Q: testResourcePrefix})
	if err != nil {
		return err
	}

	for _, u := range users {
		if err := ensureUserDelete(u.Id, u.Status, client.oktaClient, context.TODO()); err != nil {
...
func testAccCheckUserDestroy(s *terraform.State) error {
	...
	for _, r := range s.RootModule().Resources {
		if _, resp, err := client.User.GetUser(context.TODO(), r.Primary.ID); err != nil {
