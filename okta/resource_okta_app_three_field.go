...
func resourceAppThreeFieldUpdate(d *schema.ResourceData, m interface{}) error {
	client := getOktaClientFromMetadata(m)
	app := buildAppThreeField(d, m)
	_, resp, err := client.Application.UpdateApplication(getOktaContextFromMetadata(m), d.Id(), app)
...
func resourceAppThreeFieldDelete(d *schema.ResourceData, m interface{}) error {
	client := getOktaClientFromMetadata(m)
	resp, err := client.Application.DeactivateApplication(getOktaContextFromMetadata(m), d.Id())
	if err != nil {
		return responseErr(resp, err)
	}

	return responseErr(client.Application.DeleteApplication(getOktaContextFromMetadata(m), d.Id()))
}
