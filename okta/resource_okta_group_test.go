...
func sweepGroups(client *testClient) error {
	...
	groups, _, err := client.oktaClient.Group.ListGroups(context.TODO(), &query.Params{Q: testResourcePrefix})
	if err != nil {
		return err
	}

	for _, s := range groups {
		if _, err := client.oktaClient.Group.DeleteGroup(context.TODO(), s.Id); err != nil {
...
func doesGroupExist(id string) (bool, error) {
	...
	_, response, err := client.Group.GetGroup(context.TODO(), id)
