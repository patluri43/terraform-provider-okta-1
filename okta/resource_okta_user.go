	...
	return &schema.Resource{
		...
		Importer: &schema.ResourceImporter{
			State: func(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
				...
				user, _, err := client.User.GetUser(getOktaContextFromMetadata(meta), d.Id())
				if err != nil {
					return nil, err
				}
				d.SetId(user.Id)
...
func resourceUserCreate(d *schema.ResourceData, m interface{}) error {
	...
	user, _, err := client.User.CreateUser(getOktaContextFromMetadata(m), userBody, qp)
...
func resourceUserRead(d *schema.ResourceData, m interface{}) error {
	...
	user, resp, err := client.User.GetUser(getOktaContextFromMetadata(m), d.Id())
...
func resourceUserUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if userChange {
		...
		_, _, err := client.User.UpdateUser(getOktaContextFromMetadata(m), d.Id(), userBody, nil)
		if err != nil {
	...
	if passwordChange {
		...
		_, _, err := client.User.ChangePassword(getOktaContextFromMetadata(m), d.Id(), *npr, nil)
		if err != nil {
	...
	if recoveryQuestionChange || recoveryAnswerChange {
		...
		_, _, err := client.User.ChangeRecoveryQuestion(getOktaContextFromMetadata(m), d.Id(), *nuc)
...
func resourceUserDelete(d *schema.ResourceData, m interface{}) error {
	return ensureUserDelete(d.Id(), d.Get("status").(string), getOktaClientFromMetadata(m), getOktaContextFromMetadata(m))
}

func ensureUserDelete(id, status string, client *okta.Client, context context.Context) error {
	...
	for i := 0; i < passes; i++ {
		_, err := client.User.DeactivateOrDeleteUser(context, id, nil)
		if err != nil {
...
func resourceUserExists(d *schema.ResourceData, m interface{}) (bool, error) {
	...
	_, resp, err := client.User.GetUser(getOktaContextFromMetadata(m), d.Id())
